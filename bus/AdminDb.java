...
public class AdminDb {
    ...
    public int addAdmin(Admin admin) throws SQLException {
        int rowsAffected = 0;
        try (PreparedStatement pst = con.prepareStatement(Queries.insert_admin)) {
    ...
    public static int adminLogin(String username, String password) {
        int userId=-1;
        try {
            PreparedStatement pst = DatabaseConnection.getConnection().prepareStatement(Queries.admin_login);
    ...
    private void insertAdminCredentials(int userId, String username, String password) throws SQLException {
        try (PreparedStatement pst = con.prepareStatement(Queries.insert_admincred)) {
            pst.setInt(1, userId);
    ...
    public int getId(String email) throws SQLException {
        try (PreparedStatement pst = con.prepareStatement(Queries.select_adminid)) {
    ...
    public void deleteAdmin(String mail) throws SQLException {
        try (PreparedStatement pst = con.prepareStatement(Queries.admin_delete)) {
   ...
   public List<String> displayAdmin() throws SQLException {
    ...
    try (PreparedStatement pst = con.prepareStatement(Queries.display_admin);
         ResultSet rs = pst.executeQuery()) {
    ...
    public List<String> displayRoute() throws SQLException {
    List<String> routes = new ArrayList<>();
    StringBuilder formattedRoutes = new StringBuilder();
    try (PreparedStatement pst = con.prepareStatement(Queries.display_Route);
         ResultSet rs = pst.executeQuery()) {
        formattedRoutes.append("+-----+---------------------+---------------------+\n");
        formattedRoutes.append("| ID  | From                | To                  |\n");
        formattedRoutes.append("+-----+---------------------+---------------------+\n");
        while (rs.next()) {
            formattedRoutes.append(String.format("| %-3d | %-19s | %-19s |\n",
                    rs.getInt(1), 
                    rs.getString(2), 
                    rs.getString(3)));
        }
    ...
    public void addBus(Bus bus) throws SQLException {
        try (PreparedStatement pst = con.prepareStatement(Queries.insert_bus)) {
...
public List<String> displayBus(int userId) throws SQLException {
    ...
    // Query the database for the buses belonging to the user
    try (PreparedStatement pst = con.prepareStatement(Queries.display_bus)) {
        pst.setInt(1, userId);
        ResultSet rs = pst.executeQuery();

        // Iterate through each bus and display its details in a separate table
        while (rs.next()) {
